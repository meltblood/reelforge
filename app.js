...
const els = {
  ...
  modeControl: $('modeControl'), presetSelect: $('presetSelect'), randomizeBtn: $('randomizeBtn'),
...
function applyPresetValue(value){
  if(value==='randomized'){ randomizeParameterRanges(); return; }
  let preset;
...
function randomizeParameterRanges(){
  const source = currentMode==='manual' ? MODES.strong : (MODES[currentMode] || MODES.strong);
  const randomized={};
  for(const [key,def] of Object.entries(RANGE_DEFS)){
    const base=source[key] || [def.min,def.max];
    const baseMin=clamp(base[0],def.min,def.max);
    const baseMax=clamp(base[1],def.min,def.max);
    const span=Math.max(def.step,baseMax-baseMin);
    let a=rnd(baseMin,baseMax);
    let b=rnd(baseMin,baseMax);
    let lo=Math.min(a,b), hi=Math.max(a,b);
    const minWindow=Math.max(def.step,span*rnd(.16,.34));
    if(hi-lo<minWindow){
      const center=(lo+hi)/2;
      lo=clamp(center-minWindow/2,baseMin,baseMax);
      hi=clamp(center+minWindow/2,baseMin,baseMax);
      if(hi-lo<minWindow){
        if(lo<=baseMin+def.step) hi=clamp(baseMin+minWindow,baseMin,baseMax);
        else lo=clamp(baseMax-minWindow,baseMin,baseMax);
      }
    }
    randomized[key]=[round(lo,4),round(hi,4)];
  }
  renderRanges(randomized);
  els.manualDetails.open=true;
  let opt=els.presetSelect.querySelector('option[value="randomized"]');
  if(!opt){ opt=document.createElement('option'); opt.value='randomized'; opt.textContent='Случайные параметры'; els.presetSelect.appendChild(opt); }
  els.presetSelect.value='randomized';
}
...
els.randomizeBtn.addEventListener('click',randomizeParameterRanges);
